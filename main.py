...
def menu_tree(con: psycopg2.extensions.connection):
    ...
    while True:
        ...
        if choice in ("populate", "1"):
            ...
            print(f"Total amount of orders: {temp}")
        elif choice in ("total sales", "4"):
            temp=get_total_sales(con)
            print(f"Total value of all sales: {temp}")
        elif choice in ("get low stock", "5"):
            temp=get_low_stock_count(con)
            print(f"Amount of products with a stock lower than 10: {temp}")
            ...
            print("Total orders and sales by year and month:")
            for n in temp:
                print(f"  Date: {n[0].strftime('%Y.%m')}, orders {n[1]}, sales {n[2]}")
            ...
            print("Top five customers by spending:")
            for n in temp:
                print(f"  {n[0]} spent {n[1]}")
        elif choice in ("suppliers products", "11"):
            temp=get_suppliers_with_products(con)
            print("Each supplier with their amount of products:")
            for n in temp:
                print(f"  {n[0]} has {n[1]} products")
        elif choice in ("bad products", "12"):
            temp=get_products_not_sold(con)
            print("List of products with no sales:")
            for n in (temp):
                print(f"  {n[0]}")
        elif choice in ("customers above treshold", "13"):
            while True:
                try:
                    threshold = int(input("Set threshold for spending per customer: "))
                    break
                except ValueError:
                    print("Invalid input, write a number: ")
            temp=get_customers_bought_above_threshold(con, threshold)
            print(f"Customers that spent above {threshold} (limited to 25 customers max)")
            for n in temp:
                print(f"  {n[0]} {n[1]}")
        elif choice in ("orders by products", "14"):
            temp=get_orders_most_products(con)
            print("Order id and amount of products:")
            for n in temp:
                print(f"  {n[0]}: {n[1]}")
        elif choice in ("daily orders peak", "15"):
            temp=get_daily_orders_with_peak(con)
            print("Most orders by date:")
            for n in temp:
                print(f"  Date: {n[0].strftime('%Y.%m.%d')}, orders {n[1]}")
        elif choice in ("avg monthly deliverytime", "16"):
            temp=get_avg_monthly_deliverytime(con)
            print("Average delivery time per month:")
            for n in temp:
                print(f"  {n[0].strftime('%Y.%m')} delivery time in days {round(n[1])}")
        elif choice in ("percentage sales products", "17"):
            temp=get_percentage_total_sales_from_top_products(con)
            print(f"Top 10% of products account for {round(temp[0][0],2)} % of total sales")
